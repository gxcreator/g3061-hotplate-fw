...
void OLED_ShowChar(uint8_t x, uint8_t y, uint8_t chr, uint8_t sizey) {
    ...
    for (i = 0; i < size1; i++) {
        if (i % sizex == 0 && sizey != 8)
            OLED_Set_Pos(x, y++);
        if (sizey == 8)
            OLED_WR_Byte(asc2_0806[c][i], OLED_DATA); // 6X8 font
        else if (sizey == 16)
            OLED_WR_Byte(asc2_1608[c][i], OLED_DATA); // 8x16 font
        //		else if(sizey==xx) OLED_WR_Byte(asc2_xxxx[c][i],OLED_DATA);//User-defined
        // font
        else
            return;
    }
