...
void main(void) {
    ...
    while (1) {
        if (sensor_fault) {
            ...
            while (1) {
            } // Do not restart heating after a sensor fault.
        }
        if (page == 0) {
            page0();
        } // Page 0
...
INTERRUPT(timer0, EXTI_VectTimer0) {
    ...
    if (!sensor_fault && pwm > time && swclose) {
        heat = 1;
    } else /* if(pwm<time || !swclose)*/ {
        heat = 0;
    }
...
void page0(void) {
    ...
    for (i = 0; i < numofsam; i++) {
        powvol +=
            SUPPLY_VOLTAGE_SCALE * vcc * (powvol_average[i] / 4096); // Calculate supply voltage
...
void page1(void) {
    ...
    } else if (pagenum >= 100 && pagenum < 110) // PID settings page
    {
        ...
        switch (pagenum % 10) {
        case 0:
            Draw_midnum(53, 4, kp, 4);
            break;
        case 1:
            Draw_midnum(53, 4, ki, 4);
            break;
        case 2:
            Draw_midnum(53, 4, kd, 4);
            break;
        }
        for (i = 0; i < page1_PIDnum; i++) {
            if (i != pagenum % 10) {
    ...
    } else if (pagenum >= 200 && pagenum < 210) // Temperature settings page
    {
        ...
        switch (pagenum % 10) {
        case 0:
            Draw_midnum(53, 4, maxtartem, 3);
            break;
        case 1:
            Draw_midnum(53, 4, mintartem, 3);
            break;
        }
        for (i = 0; i < page1_TEMnum; i++) {
            if (i != pagenum % 10) {
...
void mode0(void) {
    ...
    if (realtem >= mintartem && realtem <= maxtartem) {
        Draw_Loading(8, 6, 0,
                     ((realtem - mintartem) * 100) /
                         (maxtartem - mintartem)); // Draw left progress bar
    } else if (realtem > maxtartem) {
        Draw_Loading(8, 6, 0, 100);
    } else {
        Draw_Loading(8, 6, 0, 0); // Draw left progress bar
    }
    if (showpwm != pwm && swclose) {
        showpwm_opp = (pwm / 10 - showpwm);
        showpwm_opp = showpwm_opp * 0.8;
        showpwm = pwm / 10 - showpwm_opp;
    } else if (!swclose) {
        showpwm *= 0.8;
    }
    Draw_Loading(65, 6, 1, showpwm + 1); // Draw right progress bar
    Draw_Sign(11, 48, ((tartem - (mintartem)) * 100) / ((maxtartem) - (mintartem)), tartem,
              realtem); // Draw left progress bar marker
    if (swclose == 0) {
        OLED_DrawBMP_2(84, 0, 42, 48, state[2]);
    } else if (err > 5) {
        OLED_DrawBMP_2(84, 0, 42, 48, state[1]);
    } else {
        OLED_DrawBMP_2(84, 0, 42, 48, state[0]);
    }
}
...
void mode1(void) {
    ...
    if (swclose == 0) {
        OLED_DrawBMP_2(4, 6, 16, 16, switch_clo);
    } else {
        OLED_DrawBMP_2(4, 6, 16, 16, switch_ope);
    }
